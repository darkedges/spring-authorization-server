...
/**
 * An {@link AuthenticationProvider} implementation for the OAuth 2.0 Pushed Authorization Request
...
public class OAuth2PushedAuthorizationRequestAuthenticationProvider implements AuthenticationProvider {
	...
	/**
	 * Sets the {@link OAuth2TokenGenerator} that generates the {@link OAuth2RequestUri}.
	 *
	 * @param pushedAuthorizationRequestGenerator the {@link OAuth2TokenGenerator} that generates the {@link OAuth2RequestUri}
	 * @since 1.0.0
	 */
	public void setPushedAuthorizationRequestGenerator(OAuth2TokenGenerator<OAuth2RequestUri> pushedAuthorizationRequestGenerator) {
	...
	/**
	 * Sets the {@code Consumer} providing access to the {@link OAuth2PushedAuthorizationRequestAuthenticationContext}
	 * and is responsible for validating specific OAuth 2.0 Authorization Request parameters
	 * associated in the {@link OAuth2PushedAuthorizationRequestAuthenticationToken}.
	 * The default authentication validator is {@link OAuth2PushedAuthorizationRequestAuthenticationValidator}.
	 *
	 * <p>
	 * <b>NOTE:</b> The authentication validator MUST throw {@link OAuth2PushedAuthorizationRequestAuthenticationException} if validation fails.
	 *
	 * @param authenticationValidator the {@code Consumer} providing access to the {@link OAuth2PushedAuthorizationRequestAuthenticationContext} and is responsible for validating specific OAuth 2.0 Authorization Request parameters
	 * @since 0.4.0
	 */
	public void setAuthenticationValidator(Consumer<OAuth2PushedAuthorizationRequestAuthenticationContext> authenticationValidator) {
	...
	public Authentication authenticate(Authentication authentication) throws AuthenticationException {
		...
		RegisteredClient registeredClient = this.registeredClientRepository.findByClientId(
				pushedAuthorizationRequestAuthentication.getClientId());
		if (registeredClient == null) {
		...
		OAuth2PushedAuthorizationRequestAuthenticationContext authenticationContext =
				OAuth2PushedAuthorizationRequestAuthenticationContext.with(pushedAuthorizationRequestAuthentication)
						.registeredClient(registeredClient)
						.build();
		this.authenticationValidator.accept(authenticationContext);
		// code_challenge (REQUIRED for public clients) - RFC 7636 (PKCE)
		String codeChallenge = (String) pushedAuthorizationRequestAuthentication.getAdditionalParameters().get(PkceParameterNames.CODE_CHALLENGE);
		if (StringUtils.hasText(codeChallenge)) {
			String codeChallengeMethod = (String) pushedAuthorizationRequestAuthentication.getAdditionalParameters().get(PkceParameterNames.CODE_CHALLENGE_METHOD);
		...
		OAuth2PushedAuthorizationRequest requestClaims =
				OAuth2PushedAuthorizationRequest.builder()
						.requestUri(requestUri.getTokenValue())
						.expiresIn(60)
						.build();
		return new OAuth2PushedAuthorizationRequestAuthenticationToken(authorizationRequest.getAuthorizationUri(),
				registeredClient.getClientId(), principal, requestClaims, redirectUri,
